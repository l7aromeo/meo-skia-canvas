...
// The npm channel's release tag. Both channels carry a prefix from npm 6.0.0
// and crate 0.16.0 on, so neither reads as the default; the crate's is
// `rust-v`.
//
// Unconditional, and right for every package that will ever carry it: a
// published package ships its own copy of this file, so 5.9.0's copy asks for
// `v5.9.0` and 6.0.0's will ask for `npm-v6.0.0`. Every release before 6.0.0
// baked `releases/download/v${version}/` into its own copy and into its
// platform packages, and those releases still answer those URLs -- so the old
// tags are frozen rather than renamed.
//
// In a working tree before 6.0.0 is tagged the two halves disagree: `version`
// in `package.json` is still a 5.x number while this file already asks for
// `npm-v${version}`, and no such tag exists. The download 404s and the install
// falls through to compiling from source, which is a release build on a fresh
// clone and on any hook run that reaches the postinstall. It ends when 6.0.0
// ships.
//
// Comparing the version here would buy that back at the price of asserting in
// the code that both schemes are live in published packages. They never are:
// each package resolves only its own version, and no published copy of this
// file sees a version from the other scheme.
const NPM_TAG = (version) => `npm-v${version}`;
...
async function snapshot() {
  ...
  let { version } = await config(),
    ...
    tag = NPM_TAG(version),
    release = releases.find((r) => r.tag_name === tag || r.name === tag);
  if (!release) {
    throw new Error(`release ${tag} not found on l7aromeo/meo-skia-canvas`);
  }
...
async function ensureRelease(version) {
  ...
  try {
    ...
    try {
      await exec(
        `gh release create ${tag} --draft --title "${tag} — Node addon" --generate-notes -R l7aromeo/meo-skia-canvas`,
      );
    } catch (raced) {
...
async function upload() {
  ...
  try {
    ...
    await exec(
      `gh release upload ${NPM_TAG(version)} ${artifact} --clobber -R l7aromeo/meo-skia-canvas`,
    );
  } catch (e) {
...
async function download(...args) {
  ...
  let { version, triplet, prebuild } = await config(),
    url = `${BINARY_HOST}/${NPM_TAG(version)}/${triplet}.gz`,

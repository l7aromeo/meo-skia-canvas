...
async function ensureRelease(version) {
  ...
  try {
    ...
    // the same one.
    if (JSON.parse(stdout).isDraft === false)
      throw Object.assign(
        Error(
...
async function download(...args) {
  ...
  try {
    ...
    // verify against the `prebuild` manifest in package.json. A repo copy carries no such key and
    // has nothing to check against, which is the only case that legitimately skips verification.
    //
    ...
    //
    // A manifest that omits this triplet is not the same as having no manifest: it is a manifest
    // that does not cover the bytes just downloaded, so it fails closed. Reading a missing entry as
    // "nothing to check" would install an unverified binary on exactly the platform the release
    // forgot, and would leave the empty `{}` an older `gh` writes -- see the comment on `snapshot`
    // -- verifying nothing at all, without saying so.
    if (prebuild) {

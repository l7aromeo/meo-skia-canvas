...
// `rust-v`.
//
// Only from 6.0.0. Every package published before it baked
// `releases/download/v${version}/` into its own copy of this file and into its
// platform packages, and 113 `v*` tags answer those URLs -- so the old tags
// are frozen rather than renamed. Nothing resolves a version other than its
// own, which is what makes the two schemes able to coexist: a 5.x install asks
// for `v5.9.0` and a 6.x install asks for `npm-v6.0.0`.

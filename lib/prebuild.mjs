...
async function snapshot() {
  ...
  let { version } = await config(),
    releases = JSON.parse(
      (
        await exec(
          `gh api repos/l7aromeo/meo-skia-canvas/releases --paginate`,
        )
      ).stdout,
    ),
    release = releases.find((r) => r.name === `v${version}` || r.tag_name === `v${version}`);
  if (!release) {
    throw new Error(`release v${version} not found on l7aromeo/meo-skia-canvas`);
  }
...
async function ensureRelease(version) {
  let tag = `v${version}`;
  try {
    await exec(`gh release view ${tag} -R l7aromeo/meo-skia-canvas`);
  } catch {
...
async function download(...args) {
  ...
  try {
    ...
    if (!args.includes("--or-compile") || !existsSync(`${ROOT}/Cargo.toml`)) throw e;
...
export { config, download, upload, snapshot, compile, BINARY_PATH, BINARY_HOST };

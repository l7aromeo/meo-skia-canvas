...
// `targets.json` is the single source of the platform list -- the same file `lib/binary.js` derives
// `PLATFORM_PACKAGES` from, and the one `npm run sync-targets` regenerates `optionalDependencies`
// from. The two Lambda archives are the other assets `snapshot` records a digest for; together they
// are exactly what a complete `prebuild` manifest contains.
const TARGETS = JSON.parse(readFileSync(`${ROOT}/lib/targets.json`, "utf8"));
...
async function snapshot() {
  ...
  let expected = Object.keys(TARGETS).length + LAMBDA_ARCHIVES.length,

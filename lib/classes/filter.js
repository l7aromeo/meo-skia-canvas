...
class ColorFilter {
  ...
  static MakeMatrix(matrix) {
    ...
    // and what Chrome raises for `new DOMMatrix([1,2,3])`. It was a
    // `RangeError`, which reads as a claim that 20 is one end of a permitted
    // range rather than the only length there is.
    if (matrix.length !== 20) {
      throw new TypeError(`Expected 20 matrix elements, got ${matrix.length}`);
    }
  ...
  static MakeHSLAMatrix(matrix) {
    // Split from the length check its sibling makes, which this ran together
    // with the type check under one `RangeError`: a caller passing `null` and
    // a caller passing nineteen numbers made different mistakes and were told
    // the same thing. Both messages now match `MakeMatrix`.
    if (matrix == null || typeof matrix.length !== "number") {
      throw new TypeError("Expected Float32Array or ArrayLike<number>");
    }
    if (matrix.length !== 20) {
  ...
  static MakeTable(table) {
    if (table == null || typeof table.length !== "number") {
      throw new TypeError("Expected Uint8Array or ArrayLike<number>");
    }
    if (table.length !== 256) {
      throw new TypeError(`Expected 256 table elements, got ${table.length}`);
    }
  ...
  static MakeTableARGB(tableA, tableR, tableG, tableB) {
    const convert = (t) => {
      ...
      if (t.length !== 256)
        throw new TypeError(`Expected 256 table elements, got ${t.length}`);

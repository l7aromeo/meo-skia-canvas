 ...
 *
 * Reads `RAW` rather than the draining accessor beside it: `reserve` has
 * already handed over anything this object had pending of its own, and the
 * record being written is by now half in the arena, so a flush here would
 * hand over an incomplete one. What the accessor exists to catch -- the
 * object being touched while a record still points at it -- is caught by
 * `referenced` instead, from the moment the record is complete.
 */
const keepHandle = (value) => {
  referenced.add(value);
  return values.push((value || {})[RAW]) - 1;
};
...
function guard(klass) {
  Object.defineProperty(klass.prototype, ø, {
    get() {
      ...
      // finds there. Both are answered the same way and both have to be:
      // reading the handle is the only way to reach this object from Rust,
      // so a drain here is a drain before anything can observe it or change
      // it.
      if (owner === this || referenced.has(this)) flush();
      return this[RAW];
    },
    enumerable: false,
    configurable: true,

...
// `op(...)` and `effect(...)` while being declared nowhere, so
// a caller could find them by introspection and depend on a shape nothing
// promises. The instance methods below are the surface.
...
const effect = (kind, path, ...args) => wrap(Path2D, path.ƒ(kind, ...args));

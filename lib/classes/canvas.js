...
class CanvasGradient extends RustClass {
  ...
  // Each of these two settings has two spellings, and they are one
  // property rather than two that agree. `colorInterpolationSpace` and
  // `hueInterpolationMethod` are the accurate pair: `"oklab"` names a
  // coordinate system rather than a method -- the mixing is a straight
  // line either way, only the room changes -- while `"longer"` names a
  // method, because hue is an angle and two stops leave two arcs to pick
  // between. `interpolation` and `hueInterpolation` are what shipped.
  //
  // The older pair is deprecated in `lib/index.d.ts` and keeps working
  // here indefinitely: nothing throws, nothing warns, and no behaviour
  // depends on which spelling is used. The deprecation is our own
  // imprecise name giving way to our own precise one, which is why it
  // does not depend on where whatwg/html issues 7947 and 8296 land --
  // those two do not agree with each other on the colour name.
  //
  // The older spelling delegates to the newer rather than repeating it,
  // so each setting has one validation site. Below that there is one
  // storage location as well: the string handed to `PROP` is the exported
  // native symbol -- `CanvasGradient_{get,set}_interpolation` is the only
  // accessor pair -- and it reads and writes a single `color_space` field
  // on the Rust struct. So the two spellings cannot drift apart; there is
  // nowhere for a second value to be kept.

  get colorInterpolationSpace() {
    return this[PROP]("interpolation");
  }

  set colorInterpolationSpace(value) {
    const valid = [
      "destination",
      "srgb",
      "srgb-linear",
  ...
  get interpolation() {
    return this.colorInterpolationSpace;
  }

  set interpolation(value) {
    this.colorInterpolationSpace = value;
  }

  get hueInterpolationMethod() {
    return this[PROP]("hueInterpolation");
  }

  set hueInterpolationMethod(value) {
  ...
  set hueInterpolation(value) {
    this.hueInterpolationMethod = value;
  }

  // No deprecated spelling beside this one, unlike the two above: they carry
  // an older name because they were renamed, and this property is new. An
  // alias here would invent a history the property does not have.
  get alphaInterpolationMethod() {
    return this[PROP]("alphaInterpolationMethod");
  }

  set alphaInterpolationMethod(value) {
    const valid = ["unpremultiplied", "premultiplied"];
    if (!valid.includes(value)) {
      throw new TypeError(`Invalid alpha interpolation: ${value}`);
    }
    this[PROP]("alphaInterpolationMethod", value);
  }

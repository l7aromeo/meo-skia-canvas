...
class Canvas extends RustClass {
  ...
  constructor(
    ...
    this.#contexts = [];
    // Declared in the types but never implemented, here or upstream. The
    // WeakMap holds the same array `#contexts` does, so it tracks pages as
    // they are added rather than snapshotting them.
    Canvas.contexts.set(this, this.#contexts);
    // `alloc` honours the gpu flag when choosing a rasterizer, but leaves the
    // `engine` property at its default, so `canvas.gpu` reported `true` while
    // `canvas.engine.renderer` said CPU. Mirror the flag so the two agree.
    if (!gpu) this.prop("engine", "cpu");
  ...
  toSharpSync({ page, matte, msaa, density = 1 } = {}) {
    ...
    const sharp = getSharp(),
      buffer = this.toBufferSync("raw", { page, matte, density, msaa });

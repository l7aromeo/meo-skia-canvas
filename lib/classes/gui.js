...
class Window extends EventEmitter {
  ...
  set canvas(canvas) {
    // A `TypeError`, and none of AGENTS.md's four rules covers it: this is a
    // value of the wrong kind rather than an enum's spelling, a sequence's
    // length or a number's range. WebIDL raises a `TypeError` when interface
    // conversion fails, which is what a browser does for `drawImage(42)`.
    if (!(canvas instanceof Canvas)) {
      throw new TypeError("Expected a Canvas for `canvas`");
    }
    {
  ...
  set fit(mode) {
    ...
    if (!css.fit(mode)) {
      throw new TypeError(
        `Expected a FitStyle keyword for \`fit\` (got ${JSON.stringify(mode)})`,
      );
    }
    this.#state.fit = mode;
  }

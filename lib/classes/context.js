...
class CanvasRenderingContext2D extends RustClass {
  ...
  fill(path, rule) {
    ...
    if (path instanceof Path2D) {
      const write = drawlist.writerFor(CanvasRenderingContext2D, "fillPath2D");
      if (write && (rule === undefined || typeof rule === "string")) {
        return write.call(this, path, rule ?? "nonzero");
      }
  ...
  measureText(text, maxWidth) {
    // An object, not a JSON string. Serialising the measurements here and
    // parsing them back was about 40 µs of the call's 73 -- more than the
    // typesetting it reports.
    return new TextMetrics(this.ƒ("measureText", toString(text), maxWidth));
  }

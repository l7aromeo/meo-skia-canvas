...
class CanvasRenderingContext2D extends RustClass {
  ...
  createProjection(quad, basis) {
    // `null` where no projection exists, mirroring the crate's `Option`
    // rather than handing back a `DOMMatrix` of NaN. A NaN matrix is worse
    // than no answer: every transform path drops a non-finite matrix in
    // silence, so applying one left the drawing untransformed with nothing
    // said.
    const values = this[CALL](
      "createProjection",
      [quad].flat(),
      [basis].flat(),
    );
    return values === null ? null : fromSkMatrix(values);
  }
  ...
  getImageData(
    ...
    width,
    height,
    { colorType, colorSpace, alphaType, density = 1, matte, msaa } = {},
  ) {
    argc(arguments, 4, 5);
    ...
    // `alphaType` is NOT resolved against the canvas the way the two above
    // are: there is nothing on a canvas to resolve it from, and an absent key
    // means the default in `ExportOptions::default` -- unpremultiplied, what
    // `putImageData` means. Passing `undefined` is what makes omitting the
    // key and writing `"unpremultiplied"` the same request rather than two.
    let opts = { colorType, colorSpace, alphaType, density, matte, msaa },
      buffer = this[CALL](
        "getImageData",
        x,
        y,

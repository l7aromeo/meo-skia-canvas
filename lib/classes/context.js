...
class CanvasRenderingContext2D extends RustClass {
  ...
  getImageData(
    ...
    width,
    height,
    { colorType, colorSpace = "srgb", density = 1, matte, msaa } = {},
  ) {
    argc(arguments, 4, 5);

    // Inherit the canvas's own format rather than hard-coding "rgba", so a canvas
    // built with {colorType} is honoured. Resolved here rather than passed through as
    // undefined because the ImageData below needs it to know the buffer's layout.

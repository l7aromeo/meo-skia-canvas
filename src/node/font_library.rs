...
mod tests {
    ...
    /// DIAGNOSTIC, TEMPORARY. Delete with
    /// `.github/workflows/diag-windows-provider.yml` once #10 is settled.
    ///
    /// Prints what `svg_font_mgr` holds for a document naming no family --
    /// the case where `4ex` renders as `2em` on Windows and nowhere else.
    /// **It asserts nothing on purpose.** A failing assertion carries one
    /// bit; the question here is which of several hypotheses survives, and
    /// that needs the values. Read it with `--nocapture`.
    ///
    /// What each line rules out:
    ///
    /// - `families` at 0 means the provider is empty and a null family resolves
    ///   to nothing, so `ex_ratio_for` takes the constant and whatever draws is
    ///   Skia's own raster-time fallback rather than ours.
    /// - a populated provider whose index 0 measures means neither the
    ///   selection nor the registration is at fault and the null path is
    ///   reaching something else -- instrument `ex_ratio_for` next, not this.
    /// - a populated provider whose index 0 does *not* measure means the face
    ///   registered first is not the one the guard approved, since selection
    ///   and measurement call the same `x_height_ratio` on the same typeface
    ///   and cannot disagree.
    /// - the curated stack's own answer separates "nothing was offered" from
    ///   "something was offered and rejected", which want different fixes.
    #[test]
    fn diagnostic_what_the_svg_provider_holds_for_an_unnamed_family() {
        let report = FontLibrary::with_shared(|lib| {
            let system = lib.mgr.clone();
            let stack: Vec<String> = lib
                .font_collection()
                .find_typefaces(
                    &["system-ui", "sans-serif", "serif"],
                    FontStyle::normal(),
            ...
            // The manager the failing document builds: the real system
            // manager, and nothing named.
            let mgr = lib.svg_font_mgr(&system, &[]);
            let families: Vec<String> = (0..mgr.count_families())
                .map(|i| mgr.family_name(i))
                .collect();
            let answered = mgr
                .legacy_make_typeface(None, FontStyle::normal())
                .map(|face| {
            ...
            format!(
                "\n  curated stack offered ({}): {:?}\n                   system default for a null family: {:?}\n                   provider families ({}): {:?}\n                   provider answer for a null family: {:?}",
                stack.len(),
                stack,
                system_default,
                families.len(),
        ...
        eprintln!("PROVIDER DIAGNOSTIC{report}");
    }

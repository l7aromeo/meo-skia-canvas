...
- Those figures are the raster backend's. Skia's Metal and Vulkan backends implement no 32-bit float surface -- only its GL and raster paths do -- so `F32` on the GPU falls back to N32 rather than failing the draw. `F16` is provided, but a GPU quantises the paint colour to eight bits before compositing, so the same sixty layers land on 0.235: different from eight-bit rather than closer to right. Metal and Vulkan return that figure to the digit, which is what makes it Skia's limit rather than a driver's. Use `gpu: false` when the arithmetic matters.

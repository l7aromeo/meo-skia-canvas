...
- animates: pages become frames in a WebP, GIF, APNG or AVIF, timed by `fps` or a per-frame `frameDelays` array. An animated image read back reports its own `frames` and `delays`, so all four are a round trip. Skia decodes neither APNG nor AVIF — it opens an APNG as the still image inside it — so this library demuxes both itself
